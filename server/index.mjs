...
app.post('/api/quotes', (request, response) => {
  ...
  const transaction = db.transaction(() => {
    ...
    db.prepare('INSERT INTO vehicles (quotation_id, type, brand, model, version, manufacture_year, model_year, zero_km, financed, has_plate, plate, alarm, anti_theft, gas_kit, taxi, tax_exempt, tax_exemption) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, data.vehicleType, data.brand, data.model, data.version, Number(data.manufactureYear), Number(data.modelYear), data.zeroKm, data.financed, data.hasPlate, data.plate || null, data.alarm, data.antiTheft, data.gasKit, data.taxi, data.taxExempt, data.taxExemption)
